...
// ----------------- Combined Thunks -----------------
export const addItemAndRefreshCart = createAsyncThunk("cart/addItemAndRefreshCart", async (payload, { dispatch }) => {
    await dispatch(addToCartBackend(payload)); // add item
    const res = await dispatch(fetchCart()); // refresh cart
    return res.payload;
});
// ----------------- Async Thunks -----------------
...
export const addToCartBackend = createAsyncThunk("cart/addToCartBackend", async (payload) => {
    const res = await api.post("cart/add_item/", payload);
    return res.data;
});
export const updateCartItemBackend = createAsyncThunk("cart/updateCartItemBackend", async (payload) => {
    const res = await api.post("cart/update_item/", payload);
    return res.data;
});
// ✅ Updated: remove uses item_id
export const removeCartItemBackend = createAsyncThunk("cart/removeCartItemBackend", async (payload) => {
...
const cartSlice = createSlice({
    ...
    reducers: {
        addToCart: (state, action) => {
            ...
            if (state.cart) {
                state.cart.cart_total = state.cart.items.reduce((sum, item) => sum + item.price * item.quantity, 0);
